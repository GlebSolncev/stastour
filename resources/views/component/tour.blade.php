...
<section class="tour">
    ...
    <div class="tour__grid">
        ...
        @if($tour->bokun_id && $tour->pricingCategory)
            <div id="tour-form">
                <div class="tour__calendar--wrapper" js-module="tour-checkout" data-id="{{$tour->bokun_id}}">
                    <tour-booking
                            :tour-id="'{{ $tour->bokun_id }}'"
                            :initial-categories="{{ json_encode(collect($tour->pricingCategory)->values()->toArray()) }}"
                            :min-date="'{{ now()->format('Y-m-d') }}'"
                    ></tour-booking>
                </div>
            </div>
{{--            @include('component.partial.tour-bokun', ['tour' => $tour])--}}
        @else
